...
const MenuSection = () => {
    ...
    return (
        <div className={`p-2 md:p-4 ${themes[theme].bg} transition-all duration-300 font-light font-mono text-xs md:text-md lg:text-lg `}>
            {/* Filter Section */}
            <div className="mb-4 flex gap-1 flex-wrap">
                {["All", "Appetizers", "Main Course", "Desserts", "Drinks"].map((category) => (
                    <button key={category} className={`px-1 text-xs md:text-sm py-.5 md:px-4 md:py-2 ${themes[theme].highlight} hover:bg-gray-300 rounded`}>
                        {category}
                    </button>
                ))}
            </div>
            ...
            {isAdmin && (
                <div className="flex justify-between  w-full items-center flex-wrap">
                    <div onClick={() => {
                        ...
                        className={`px-1 text-xs md:px-4 py-1 md:py rounded-sm border border-gray-500 shadow-sm cursor-pointer ${theme === "light" ? "bg-gray-300 text-black" : ""} ${theme === "dark" ? "bg-gray-600 text-white" : ""} ${theme === "maroon" ? "bg-[#FFD700] text-black" : ""}`}
                    >
                        Add Food
                    </div>
                    ...
                    {/* Theme Selector */}
                    <select
                        className={`px-1 md:px-4 py-1 rounded-md text-xs border border-gray-500 shadow-md cursor-pointer ${theme === "light" ? "bg-gray-300 text-black" : ""} ${theme === "dark" ? "bg-gray-600 text-white" : ""} ${theme === "maroon" ? "bg-[#FFD700] text-black" : ""}`}
                        value={theme}
                        onChange={(e) => setTheme(e.target.value)}
                    >
                        <option value="light" className="font-mono font-light text-xs md:font-md lg:font-ld"> Light</option>
                        <option value="dark" className="font-mono font-light text-xs md:font-md lg:font-ld"> Dark</option>
                        <option value="maroon" className="font-mono font-light text-xs md:font-md lg:font-ld" > Maroon</option>
                    </select>
